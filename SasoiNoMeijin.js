...
@keyframes sasoiScoreCornerFadeIn{
...
@keyframes sasoiScoreNumberCenterShow{
...
@keyframes sasoiScoreTitleCenterShow{
...
//
// プレイごとに番号を更新する。
// リトライした場合、前回の魚アニメーションを
// 次のプレイへ持ち越さないために使用する。
// =================================

let sasoiCatchAnimationSessionId =
  0;
...
let sasoiScoreTitleAnimationTimer =
  null;
...
const SASOI_BITE_LATE_END = 340;


// 340ms以上
// → 遅すぎてLOST
const SASOI_BITE_TOO_LATE = 340;
...
<div class="sasoi-title" id="sasoiDebugCommandArea">誘いの名人</div>
...
// ---------------------------------
// 現在の音符が
// 2個目以降のhold または biteなら
// 見た目だけ隠す。
// ---------------------------------

if(
noteIndex > firstHoldIndex &&
noteIndex <= biteIndex
){
...
//
// リトライ時に前回の
// .is-visible が残らないようにする。
// 中央タイトル演出が終わるまで
// 右上は非表示に戻す。
// ---------------------------------

const sasoiCornerName =
  document.querySelector(
    ".sasoi-score-name"
  );
...
if(
  ...
  sasoiCornerName.classList.remove(
    "is-visible"
  );
...
if(
  ...
  const oldCatchFish =
    catchDisplay.querySelectorAll(
      ".sasoi-catch-fish"
    );


  oldCatchFish.forEach(
    function(fish){
...
//
// 魚がケースに入った瞬間に
// sasoiTotalCatchCount を +1 する。
//
// ---------------------------------
//
// リトライされた場合は、
// その前のプレイで開始した魚アニメーションを
// 次のプレイへ持ち越さない。
//
// =================================
...
let sasoiTenSurpriseActivated = false;
